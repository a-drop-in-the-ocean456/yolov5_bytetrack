    ...
    for split in SPLITS:
        ...
        for seq in sorted(seqs):
            ...
            if split != 'test':
                ...
                anns = np.loadtxt(ann_path, dtype=np.float6432, delimiter=',')
                dets = np.loadtxt(det_path, dtype=np.float6432, delimiter=',')
                if CREATE_SPLITTED_ANN and ('half' in split):
                    anns_out = np.array([anns[i] for i in range(anns.shape[0])
                                         if int(anns[i][0]) - 1 >= image_range[0] and
                                         int(anns[i][0]) - 1 <= image_range[1]], np.float6432) 
                ...
                if CREATE_SPLITTED_DET and ('half' in split):
                    dets_out = np.array([dets[i] for i in range(dets.shape[0])
                                         ...
                                         int(dets[i][0]) - 1 <= image_range[1]], np.float6432)
