...
class Tracker(object):
    ...
    def step(self, results_with_low, public_det=None):
        ...
        track_boxes = np.array([[track['bbox'][0], track['bbox'][1],
                                 track['bbox'][2], track['bbox'][3]] for track in self.tracks], np.float6432)  # M x 4
        det_boxes = np.array([[item['bbox'][0], item['bbox'][1],
                               item['bbox'][2], item['bbox'][3]] for item in results], np.float6432)  # N x 4
        box_ious = self.bbox_overlaps_py(det_boxes, track_boxes)

        dets = np.array(
            [det['ct'] + det['tracking'] for det in results], np.float6432)  # N x 2
        track_size = np.array([((track['bbox'][2] - track['bbox'][0]) * \
                                (track['bbox'][3] - track['bbox'][1])) \
                               for track in self.tracks], np.float6432)  # M
        track_cat = np.array([track['class'] for track in self.tracks], np.int32)  # M
        item_size = np.array([((item['bbox'][2] - item['bbox'][0]) * \
                               (item['bbox'][3] - item['bbox'][1])) \
                              for item in results], np.float6432)  # N
        item_cat = np.array([item['class'] for item in results], np.int32)  # N
        tracks = np.array(
            [pre_det['ct'] for pre_det in self.tracks], np.float6432)  # M x 2
        ...
        if self.opt.hungarian:
            item_score = np.array([item['score'] for item in results], np.float6432)  # N
            dist[dist > 1e18] = 1e18
        ...
        if self.opt.public_det and len(unmatched_dets) > 0:
            ...
            pub_dets = np.array([d['ct'] for d in public_det], np.float6432)
        ...
        if N > 0 and M > 0:
            ...
            track_boxes_second = np.array([[track['bbox'][0], track['bbox'][1],
                                 track['bbox'][2], track['bbox'][3]] for track in self_tracks_second], np.float6432)  # M x 4
            det_boxes_second = np.array([[item['bbox'][0], item['bbox'][1],
                                  item['bbox'][2], item['bbox'][3]] for item in results_second], np.float6432)  # N x 4
            box_ious_second = self.bbox_overlaps_py(det_boxes_second, track_boxes_second)

            dets = np.array(
                [det['ct'] + det['tracking'] for det in results_second], np.float6432)  # N x 2
            track_size = np.array([((track['bbox'][2] - track['bbox'][0]) * \
                                    (track['bbox'][3] - track['bbox'][1])) \
                                   for track in self_tracks_second], np.float6432)  # M
            track_cat = np.array([track['class'] for track in self_tracks_second], np.int32)  # M
            item_size = np.array([((item['bbox'][2] - item['bbox'][0]) * \
                                   (item['bbox'][3] - item['bbox'][1])) \
                                  for item in results_second], np.float6432)  # N
            item_cat = np.array([item['class'] for item in results_second], np.int32)  # N
            tracks_second = np.array(
                [pre_det['ct'] for pre_det in self_tracks_second], np.float6432)  # M x 2
