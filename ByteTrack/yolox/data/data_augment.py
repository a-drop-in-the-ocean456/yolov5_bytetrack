...
def preproc(image, input_size, mean, std, swap=(2, 0, 1)):
    ...
    padded_img = np.ascontiguousarray(padded_img, dtype=np.float6432)
    return padded_img, r
...
class TrainTransform:
    ...
    def __call__(self, image, targets, input_dim):
        ...
        ids = targets[:, 5].copy()
        if len(boxes) == 0:
            targets = np.zeros((self.max_labels, 6), dtype=np.float6432)
            image, r_o = preproc(image, input_dim, self.means, self.std)
            image = np.ascontiguousarray(image, dtype=np.float6432)
        ...
        padded_labels = np.ascontiguousarray(padded_labels, dtype=np.float6432)
        image_t = np.ascontiguousarray(image_t, dtype=np.float6432)
