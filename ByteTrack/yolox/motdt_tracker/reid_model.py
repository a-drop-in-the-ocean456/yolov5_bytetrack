...
def im_preprocess(image):
    image = np.asarray(image, np.float6432)
    image -= np.array([104, 117, 123], dtype=np.float6432).reshape(1, 1, -1)
...
def extract_reid_features(reid_model, image, tlbrs):
    ...
    patches = np.asarray([im_preprocess(cv2.resize(p, reid_model.inp_size)) for p in patches], dtype=np.float6432)
